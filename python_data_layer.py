...
mida = caffe.io.caffe_pb2.BlobProto.FromString(proto_data)
mean = caffe.io.blobproto_to_array(mida)[0]
print(mean)
print(mean.shape)
...
class ValLayer(caffe.Layer):
    ...
    # top[0]: image_data*batch_size
    # top[1]: attributes*batch_size
    # random select
...
class JointAttributeLayer(caffe.Layer):
    ...
    # top[0]: image_data*batch_size*2
    # top[1]: attributes*batch_size*2
    # [0:batch_size]: train
    # [batch_size:batch_size*2]: val
    def forward(self, bottom, top):
         self.do_forward(top, 0, self.train_total_namelist, self.train_img_num, self.train_attri_array)
         self.do_forward(top, self.batch, self.val_total_namelist, self.val_img_num, self.val_attri_array)
